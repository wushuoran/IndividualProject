...
public class AddFriendsAdapter extends BaseAdapter {
    ...
    private HashMap<Integer, String> historyUsers;

    public AddFriendsAdapter(Context context, HashMap<Integer, String> historyUsers){
        ...
        this.historyUsers = new HashMap<>();
        this.historyUsers = historyUsers;
        System.out.println(historyUsers);
    }
    ...
    public int getCount() {
        ...
        System.out.println("will create " + historyUsers.size() + " items");
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        holder.btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                builder.setMessage("Send friend request to " + finalTV.getText() + "?").setTitle("Confirm").setCancelable(true);
                builder.setPositiveButton("Send", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        Map<String, Object> friendReq = new HashMap<>();
                        friendReq.put(mAuth.getCurrentUser().getUid(), false); // a request awaiting approve
                        mStore.collection("UserFriends").document(uid).set(friendReq, SetOptions.merge()) // request add to receiver's friend list with false value
                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                            @Override
                            public void onComplete(@NonNull Task<Void> task) {
                                if (task.isSuccessful()){
                                    Toast.makeText(FindFriendsActivity.findFri, "Request sent", Toast.LENGTH_SHORT).show();
                                    btn2.setEnabled(false);
                                }else {
                                    Toast.makeText(FindFriendsActivity.findFri, "Request didn't send", Toast.LENGTH_SHORT).show();
                                }
                            }
                        });
